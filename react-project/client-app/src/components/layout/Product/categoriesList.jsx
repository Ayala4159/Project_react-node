...
import rm from '../../../pics/backCategories.webp'
...
const CatgoriesList = () => {
    ...
    const getProductsByCategory = async () => {
        try {
            const { data } = await axios.get(`http://localhost:1135/api/product/sort/${name}`)
            setProducts(data)
        } catch (err) {
    ...
    const deleteProduct = async (id) => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`http://localhost:1135/api/product/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    return (
        <>
            ...
            {role2 === "manager" && <AddProduct getProductsByCategory={getProductsByCategory} />}
            {visible && <UpdateProduct getProductsByCategory={getProductsByCategory} curProduct={curProduct} visible={visible} onHide={() => setVisible(false)} />}
